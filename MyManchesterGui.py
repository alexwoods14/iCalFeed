...
#keys = ["DTSTAMP","SUMMARY","LOCATION","DESCRIPTION","TZID","DTSTART","DTEND","RRULE","EXDATE","UID"]
        ...
        start = eventType.index('\\n')
        end = eventType[(start + 2):].index('\\n')
        eventType = eventType[start + 2: start + end + 2].capitalize()
        #

        #format location
        location = event["LOCATION"].strip("LOCATION:").replace("_",":").replace("\\","")[:20]

        if "lecture" in eventType.lower():
            colour = "#2EF"
        if "laboratory" in eventType.lower():
            colour = "#2BF"
        if "tutorial" in eventType.lower():
            colour = "#29F"
        if "example" in eventType.lower():
            colour = "#2BC"
        if "pass" in eventType.lower():
            colour = "#8FF"
